...
import 'package:infinite_shop/app/core_impl/exception/common_error.dart';
import 'package:infinite_shop/core/local_storage/local_storage.dart';
import 'package:infinite_shop/core/reactive/obs_data.dart';
import 'package:infinite_shop/core/reactive/obs_setting.dart';
import 'package:infinite_shop/core/reactive/simple_obs_data.dart';
...
ObsSetting<T> listenableSetting<T>(
  ...
  ReactiveContext? context,
  String? name,
}) => ObsSetting(initValue: initValue, key: key, name: name);
  ...
  Future<void> assignTo(
    ...
    try {
      ...
      if (loadingStatus == null) {
        ...
        newStatus =
            observableData.status == ObsDataStatus.initial
      ...
      if (observableData.status == ObsDataStatus.loadingMore) {
        ...
        (newPage as Pagination).items.insertAll(
          0,
          (observableData.value as Pagination?)?.items ?? [],
        );
